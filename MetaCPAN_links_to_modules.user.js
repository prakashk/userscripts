...
var metacpan_img = '<img width="12" height="12" style="vertical-align: middle; padding: 1px" src="' + metacpan_icon + '">';
...
function GM_wait() {
    if (typeof unsafeWindow.jQuery == 'undefined') {
        ...
        $ = unsafeWindow.jQuery;
        if (window.location.host == 'cpants.charsbar.org') {
          setup_metacpan_links();
        }
        convert_search_cpan_links();
    }
}
GM_wait();
...
//
// http://search.cpan.org/search?mode=module&query=String%3A%3AMkPasswd

function convert_search_cpan_links() {
  $('a[href^="http://search.cpan.org"]').each(
    function() {
      var module = this.href.match('(?:perldoc\\?|/dist/|search\\?mode=module&query=)(.*?)/?$')[1].replace('-', '::', 'g');
      console.log("module = " + module);
      $(this).after('&nbsp;' + '<a href="http://metacpan.org/module/' + module + '">' + metacpan_img + '</a>');
    });
}